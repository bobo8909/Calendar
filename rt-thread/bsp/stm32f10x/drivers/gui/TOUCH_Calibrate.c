...
unsigned char touchCalibrated;/*1已经校准过，0未校准*/
  ...
  do {
    ...
    if (State.Pressed == Pressed) {
      ...
      do {
        ...
        if (State.Pressed != Pressed) {
          break;
        } else if ((GUI_GetTime() - 5) > TimeStart) {
          return;
        }
      } while (1);
    }
...
static void _Explain(void) {
  _DispStringCentered("Begin to calibrated touch screen\n"
                      "Please press the touch\n"
                      "screen to continue...");
  GUI_DispStringHCenterAt("王凯", LCD_GetXSize() / 2, 5);
  _WaitForPressedState(1);
  _WaitForPressedState(0);
}
/*********************************************************************
